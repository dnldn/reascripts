...
function main()
    ...
    if note.count > 2 then
        ...
        for i = 0, note.count-1 do
            ...
            reaper.MIDI_SetNote(note.take, note.idx[i], note.sel[i], note.mute[i], shuffledNoteStart[i], shuffledNoteEnd[i], note.chan[i], note.pitch[i], note.vel[i], true)
        end
    end
end
...
undo:Begin()
main()
undo:End("DNLDN Shuffle order of selected notes (maintain original lengths per position)")